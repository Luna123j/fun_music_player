    ...
    <div>
      ...
      {(historyList.length !== 0) &&
        historyList.map((item, index) => {
          return (<div key={index}><button onClick={() => historyPlayHandler(item)}> <i className="fa-solid fa-play"></i></button><img src={item.cover} alt={item.title} />{item.title}{item.artist}</div>)
        })
      }
